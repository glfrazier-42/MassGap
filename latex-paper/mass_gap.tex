...
physical explanation.  We propose that the gap is the signature of a
...
form black holes; and LIGO merger remnants exhibit spin-dependent mass
...
masses cluster below ${\sim}2.5\;M_{\odot}$---the heaviest in a
sample of XX is YY~$M_{\odot}$
%% XX = number of NS with dynamical masses (stellarcollapse.org)
%% YY = mass of heaviest NS (stellarcollapse.org)
\citep{OConnorOtt2010}---while black holes in X-ray binaries begin at
${\sim}$ZZ~$M_{\odot}$
%% ZZ = lightest X-ray binary BH mass (stellarcollapse.org)
...
transition: when a neutron star exceeds the
Tolman--Oppenheimer--Volkoff stability limit, its matter collapses
from neutron degeneracy to quark degeneracy, producing a step-function
increase in compactness and gravitational mass.  This explanation
requires that black holes are not singularities but structured
bodies---objects with a real equation of state, finite volume, and
internal pressure inside the event horizon.  This is a testable
requirement: the TOV equations show that pressure contributes to
gravitational mass, a mechanism that cannot operate in a dimensionless
point.  If black holes have internal structure, their mergers should
exhibit spin-dependent mass deficits beyond what gravitational-wave
radiation alone predicts; we show that they do.  We present four lines
of evidence: the TOV equations predict the gap quantitatively
(\S\ref{s:tov}); massive pulsars and the pulsar population show the
expected spin-dependent disappearance (\S\ref{s:collapse}); failed
supernovae independently confirm sub-$5\;M_{\odot}$ collapse
(\S\ref{ss:rsg}); and LIGO merger remnants carry spin signatures
consistent with structured matter, not point singularities
(\S\ref{s:gwtc}).
...
stability limit (\S\ref{ss:pulsar-masses}), and the full pulsar
population thins at long periods in the pattern this predicts
...
predicts---using a single parameter fit from one test and validated
against a second (\S\ref{s:gwtc}).
...
data at all---suggests that the result is not an artefact of template
assumptions.
