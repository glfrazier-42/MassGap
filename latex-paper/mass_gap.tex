...
% Figure/table note command (descriptive paragraph below caption title)
\newcommand{\note}[1]{\par\smallskip\footnotesize\itshape #1}

% Run-in paragraph heading (used for Test 1, Test 2 in \S\ref{s:gwtc})
...
\shorttitle{Quark Degeneracy and the Lower Mass Gap}
\shortauthors{Frazier}

\author[gname=Gregory,sname=Frazier]{Gregory L. Frazier}
\affiliation{Independent Researcher, Highland, Maryland, USA.}
\email[show]{glfrazier@alum.mit.edu}
...
masses cluster below ${\sim}2.5\,\Msun$ \citep{LattimerNSmasses},
...
\end{equation}
%
contains terms in which the pressure~$P$ itself acts as a
...
in the equation of state (EOS).  For neutron-star matter, we use six
finite-temperature nuclear EOS tables from the
\texttt{stellarcollapse.org} repository \citep{OConnorOtt2010},
evaluated on the cold, beta-equilibrated slice: LS180, LS220, and
LS375 \citep{LattimerSwesty1991}, SFHo
\citep{SteinerFischerHempel2013}, DD2 \citep{TypelRopkeKlaehn2010},
and HShen \citep{Shen1998}.  These span the range from soft (LS180, $K
= 180$~MeV) to stiff (LS375, $K = 375$~MeV) nuclear
incompressibilities, producing maximum neutron-star masses of roughly
$2.0$--$3.0\,\Msun$.  For quark-star matter we use the MIT bag model
\citep{Chodos1974} with three representative bag constants, $B = 60$,
$100$, and $160\,\text{MeV}/\text{fm}^{3}$, spanning the range from
soft to stiff quark matter.  These produce maximum gravitational
masses of roughly $2.0$, $1.6$, and $1.2\,\Msun$, respectively.  While
pure quark stars remain theoretical, recent Bayesian analyses of
neutron-star observations find strong evidence for quark-matter cores
for neutron-stars above ${\sim}2\,\Msun$ \citep{Annala2020}.

%% \begin{figure}[t]
%%   \centering
%%   \includegraphics[width=\columnwidth]{tov_mass_radius.png}
...
\subcaptionbox{Gravitational Mass vs. Radius\label{fig:tov_mr}}{
  \includegraphics[width=0.5\linewidth]{tov_mass_radius.png}
}\hfill
\subcaptionbox{Gravitational Mass vs. Baryonic Mass\label{fig:tov_mg_mb}}{
  \includegraphics[width=0.37\linewidth]{tov_mg_vs_mb.png}
}
...
%% \begin{figure}[t]
%%   \centering
%%   \includegraphics[width=\columnwidth]{tov_mg_vs_mb.png}
%%   \caption{Gravitational Mass $\Mg$ vs. Baryonic Mass $\Mb$}
%%   \note{Same set of
%%     EOS as Fig.~\ref{fig:tov_mr}.  The dotted line is the Newtonian
%%     limit $\Mg = \Mb$.  Neutron-star curves (solid) lie just above
%%     this line, with pressure contributing ${\sim}10$--15\% of~$\Mg$.
...
%%     (which generates pressure) appears entirely in $\Mg - \Mb$.
...
and ${\sim}5\,\Msun$ \citep{Ozel2010,Farr2011}---is the
...
\citep{OConnorOtt2010}, cross-matched with spin periods from the
Australia Telescope National Facility (ATNF) Pulsar Catalogue
\citep{Manchester2005}.  The sample spans $1.18$--$2.74\,\Msun$ and
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{pulsar_mass_period.png}
  ...
  \note{Examined \pulsarCount{} pulsars with dynamical mass
    ...
    pulsar above $1.6\,\Msun$ has a slower period than
    80~ms. (b)~Period range per mass-bin: horizontal segments span
...
\subcaptionbox{Density per log-decade\label{fig:pop_logbins}}{%
  \includegraphics[scale=0.4]{pulsar_population_logbins.png}%
}\hfill
\subcaptionbox{Fixed-width bins vs.\ model\label{fig:pop_linear}}{%
  \includegraphics[scale=0.4]{pulsar_population_linear.png}%
}
...
$25\,\Msun$ \citep{WoosleyHegerWeaver2002, Sukhbold2016}.  This is the
...
%==============================================================================
\section{Black Hole Merger Spin Analysis}\label{s:gwtc}
%==============================================================================
...
with its Kerr analogue \citep{BardeenPressTeukolsky1972}:
%
...
The minimum component masses in the sample are $m_1 = 8.4\,\Msun$ and
$m_2 = 5.1\,\Msun$. For each event, we compute the remnant
spin~$\chi+f$ from the numerical-relativitiy fitting formula
%
...
shows that pulsars are disappearing from the population, consistent
...
mass well below $5\,\Msun$ collapse to form black holes
...
and applied to a second (\S\ref{s:gwtc}).
...
catalogs: the ATNF Pulsar Catalogue \citep{Manchester2005}, the
GWTC-3 gravitational-wave catalog \citep{GWTC3,GWOSC2023}, and
equation-of-state tables from \texttt{stellarcollapse.org}
\citep{OConnorOtt2010}. Analysis code is available at
https://github.com/glfrazier-42/MassGap.
