\documentclass[aps,prl,twocolumn,groupedaddress,longbibliography]{revtex4-2}
...
\newcommand{\Mg}{M_{\mathrm{g}}}
\newcommand{\Mb}{M_{\mathrm{b}}}

% Figure note command (descriptive paragraph below caption title)
\newcommand{\note}[1]{\par\smallskip\footnotesize\itshape #1}
...
\input{pulsar_macros}
\input{population_macros}
\input{merger_macros}
\input{eta_confound_macros}

\begin{document}

\title{Quark Degeneracy and the Lower Mass Gap}

\author{Gregory L. Frazier}
\email{glfrazier@alum.mit.edu}
\affiliation{Independent Researcher, Highland, Maryland, USA. glfrazier@alum.mit.edu}

\begin{abstract}
The observed deficit of compact objects between ${\sim}2.5$ and
${\sim}5\,\Msun$---the lower mass gap---has no widely
...
(2)\,pulsars above the $2\,\Msun$ stability limit are observed
only at fast spin rates, consistent with centrifugal support delaying
collapse to black hole;
(3)\,failed supernovae confirm that cores well below $5\,\Msun$
collapse; and
...
${\sim}2\,\Msun$ in size collapse to form black holes, and
the smallest observed black hole is over $5\,\Msun$. There is
...
holes are not singularities.
\end{abstract}

\keywords{neutron stars, black holes, mass gap, quark stars, pulsars,
...
masses cluster below ${\sim}2.5\,\Msun$ \cite{LattimerNSmasses},
while black holes in X-ray binaries begin near ${\sim}5\,\Msun$
\cite{Bailyn1998}.  The deficit of objects between these limits is
...
black holes above ${\sim}5\,\Msun$; \citet{Ozel2010} argued on
...
$2.6\,\Msun$ secondary sits squarely in the gap
\cite{GW190814}---and population synthesis studies
\cite{Zevin2020} have renewed the question of whether the gap is
...
TOV equations predict the gap;
massive pulsars and the pulsar population show the expected
spin-dependent disappearance;
failed supernovae independently confirm sub-$5\,\Msun$ collapse of
objects to black holes;
and LIGO merger remnants carry spin signatures consistent with
structured matter, in contrast to point singularities.
...
\subcaptionbox{Gravitational Mass vs.\ Radius\label{fig:tov_mr}}{%
  \includegraphics[width=0.48\linewidth]{tov_mass_radius.png}%
}\hfill
\subcaptionbox{Gravitational Mass vs.\ Baryonic Mass\label{fig:tov_mg_mb}}{%
  \includegraphics[width=0.48\linewidth]{tov_mg_vs_mb.png}%
}
...
\note{Solid curves are the six nuclear EOS; the
    ...
    baryonic mass $\Mb$, quark stars have greater gravitational mass
    $\Mg$ than do neutron stars.
}
\label{fig:tov_figs}
\end{figure*}

The Tolman--Oppenheimer--Volkoff equations
\cite{Tolman1939,OppenheimerVolkoff1939} govern the hydrostatic
...
$\Mg > \Mb$.
We define $\Mg$ as the total gravitational mass enclosed at the
stellar surface.  The baryonic mass~$\Mb$ is defined so as to
...
For neutron-star matter, $\Mb$ is the integral of the baryon
...
For quark-star matter, $\Mb$ is the integral of the bag vacuum
...
difference $\Mg - \Mb$ then measures pressure's contribution to
...
\texttt{stellarcollapse.org} repository \cite{OConnorOtt2010},
evaluated on the cold, beta-equilibrated slice: LS180, LS220, and
LS375 \cite{LattimerSwesty1991}, SFHo \cite{SteinerFischerHempel2013},
DD2 \cite{TypelRopkeKlaehn2010}, and HShen \cite{Shen1998}.  These
span the range from soft (LS180, $K = 180$~MeV) to stiff (LS375,
$K = 375$~MeV) nuclear incompressibilities, producing maximum
neutron-star masses of roughly $2.0$--$3.0\,\Msun$.  For
quark-star matter we use the MIT bag model \cite{Chodos1974} with
...
$2.0$, $1.6$, and $1.2\,\Msun$, respectively.  While pure quark
...
$\Mg$ against~$\Mb$.  Every TOV solution sits above the
$\Mg = \Mb$ line; the departure grows with compactness.  The
neutron-star curves (solid) hug the diagonal, with the pressure
contribution amounting to ${\sim}10$--15\% of~$\Mg$ at maximum mass.
...
$\Mg/\Mb \sim 1.5$--2, meaning that pressure-generated energy
...
to~$\Mg$ is larger: the gravitational mass jumps upward.  The lower
mass gap---the observed deficit of compact objects between ${\sim}2.5$
and ${\sim}5\,\Msun$ \cite{Ozel2010,Farr2011}---is the
signature of this transition, spanning from the pre-collapse
neutron-star~$\Mg$ to the post-collapse quark-star~$\Mg$.
...
5\,$\Msun$, suggesting that black holes have a pressure
component to their $\Mg$, which in turn implies that they are coherent
stellar objects---they are quark stars.

\begin{figure*}[t]
\subcaptionbox{Pulsar Mass vs.\ Spin Period\label{fig:pulsar_mass_scatter}}{%
  \includegraphics[width=0.48\linewidth]{pulsar_mass_scatter.png}%
}\hfill
\subcaptionbox{Spin Period Ranges\label{fig:pulsar_mass_bins}}{%
  \includegraphics[width=0.48\linewidth]{pulsar_mass_bins.png}%
}
  \caption{The Periods of Pulsars with Known Mass.}  \note{Examined
    \pulsarCount{} pulsars with dynamical mass measurements. No pulsar
    above $1.6\,\Msun$ has a slower period than 80~ms. Binning the
    pulsars by mass, the fastest periods are comparable across the
...
a steepening $\Mg$--$\Mb$ relationship with increasing baryonic
mass, and quark stars consistently exhibit a far larger pressure
contribution to~$\Mg$ than neutron stars at comparable~$\Mb$.

This pressure contribution to~$\Mg$ is not merely a theoretical
...
We assemble a sample of \pulsarCount{} pulsars with dynamically
measured masses from the \texttt{stellarcollapse.org} compilation
\cite{OConnorOtt2010}, cross-matched with spin periods from the
Australia Telescope National Facility (ATNF) Pulsar Catalogue
\cite{Manchester2005}.  The sample spans $1.18$--$2.74\,\Msun$ and
includes four binary categories: NS--WD (21 systems), NS--NS (7),
X-Ray/Optical (2), and NS--MS (3).  Three of the most precisely
measured high-mass pulsars---J0740+6620 ($2.08 \pm 0.07\,\Msun$;
\citealt{Cromartie2020}), J0348+0432 ($2.01 \pm 0.04\,\Msun$), and
J1614$-$2230 ($1.928 \pm 0.017\,\Msun$)---anchor the high end of the
distribution.
...
$1.0$--$1.5\,\Msun$ bin down to \pulsarSlowestTop~ms above
$2.5\,\Msun$.  Massive pulsars are found exclusively at fast spin
...
\cite{Manchester2005} and restrict the sample to the \popNormal{}
...
$\text{d}P$ scales as $P\,\text{d}P$, and in a steady-state population with
constant birth rate the number density per log-decade of period should
be non-decreasing.

\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{pulsar_population_linear.png}
  \caption{Period Distribution of Normal Pulsars ($P \geq 100$~ms).}
  \note{Observed counts in \popLinearBinWidth{}~ms bins (blue)
    compared with the steady-state dipole spin-down prediction (count
    $\propto P$, red step line).  Under magnetic dipole braking the
    observed population should rise with period; instead it falls
    exponentially, with the deficit exceeding an order of magnitude by
    3~s.}
\label{fig:pop_linear}
\end{figure}

Figure~\ref{fig:pop_linear} tests this prediction.  The observed
counts fall exponentially while the steady-state model rises linearly;
by 3~s the gap exceeds an order of magnitude.  The decline begins well
below the nominal radio death line ($P \gtrsim 5$~s), and is
distributed across the entire period range, not confined to a
high-period tail.
...
critical spin-down power \cite{FaucherGiguereKaspi2006}.  However,
...
The collapse hypothesis of the preceding subsection offers a natural
...
line---precisely the pattern seen in Figure~\ref{fig:pop_linear}.
It further predicts that the deficit should be strongest among the
most massive pulsars, a test that targeted mass measurements at long
periods could address.
...
compact objects with baryonic mass well below $5\,\Msun$ are
...
roughly $8$--$25\,\Msun$.  Stars in this range are expected to
...
massive than $16.5 \pm 1.5\,\Msun$ had been identified---despite
RSGs with masses up to ${\sim}25\,\Msun$ being well documented
...
\cite{Smartt2015}.  This discrepancy---the absence of supernova
progenitors in the ${\sim}17$--$25\,\Msun$ range---is known as
...
${\sim}1.3$--$1.8\,\Msun$ across the entire range from 8 to
$25\,\Msun$ \cite{WoosleyHegerWeaver2002,Sukhbold2016}.  This is the
mass that actually undergoes gravitational collapse; subsequent
accretion of envelope material occurs only after the compact remnant
has formed.  Thus, the baryonic mass of the collapsing core is
certainly less than $5\,\Msun$ and most likely less than $2\,\Msun$.
...
$5\,\Msun$---most likely ${\sim}1.5\,\Msun$---can and do
collapse to form black holes.  This is precisely the process
proposed above for pulsars that lose centrifugal support as they spin
down.  That the same phenomenon---black hole formation from a
${\sim}2\,\Msun$ compact core---appears in two completely
...
the TOV analysis above.

%==============================================================================
\section{Observational Evidence for Black Hole Structure}\label{s:gwtc}
%==============================================================================

The preceding sections established that compact objects with baryonic
mass well below $5\,\Msun$ collapse to form black holes, and that
...
point-singularity model.  The GWTC catalog \cite{GWTC3,GWOSC2023}
...
therefore reduces the gravitational mass~$\Mg$ relative to the conserved
baryonic mass~$\Mb$.  No baryonic mass is converted to energy---the
...
Both models predict that the mass deficit depends on the symmetric
mass ratio $\eta = m_1 m_2 / (m_1 + m_2)^{2}$, because the orbital
dynamics that govern both GW radiation and angular momentum transfer
are controlled by~$\eta$.  We use the spinning-GR prediction as a
baseline, extending \citet{BuonannoKidderLehner2008} by replacing the
Schwarzschild innermost stable circular orbit (ISCO) binding energy
with its Kerr analogue \cite{BardeenPressTeukolsky1972}:
%
\begin{equation}\label{eq:frac_gr}
  f_{\rm GR}(\eta,\chi_{\rm eff})
  = e_{\rm ISCO}(\chi_{\rm eff})\,\eta + 0.498\,\eta^{2}\,,
\end{equation}
%
where $e_{\rm ISCO}(\chi_{\rm eff})$ is the analytic Kerr ISCO binding
energy fraction; at $\chi_{\rm eff}=0$ it equals $1-\sqrt{8/9}\approx
0.0572$, recovering the BKL non-spinning formula.  The quark star model
predicts additional deficit from spin-induced $\Mg$ reduction,
proportional to the remnant spin generated.
...
The minimum component masses in the sample are $m_1 =
8.4\,\text{M}_\odot$ and $m_2 = 5.1\,\text{M}_\odot$.  For each event
we compute the remnant spin~$\chi_f$ from the numerical-relativity
fitting formula
%
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{bh_merger_deficit_per_m2_v2.png}
  \caption{Mass Deficit per Unit $m_2$ vs.\ Mass Ratio.}
  \note{Blue points: observed deficit divided by $m_2$ for each GWTC
    event.  Red dashed curve: singularity model (spinning GR,
    Eq.~\ref{eq:frac_gr}, at median $\chi_{\rm eff}$).  Purple solid curve: quark star model,
    adding spin-induced $\Mg$ reduction with $\alpha = \gwAlphaQS$
...
A natural concern is that the Test~1 correlation might partly reflect
shared $\eta$-dependence: $\chi_f$ contains $L_{\rm orb}(\eta)$ and
$f_{\rm GR}$ also depends on~$\eta$.  In the GWTC sample, however,
$\chi_f$ is dominated by the progenitor spin component
$\chi_{\rm eff}(1+q^2)/(1+q)^2$ (Spearman $\rho = \ecRhoChifSpinChif$
with $\chi_f$) rather than by $L_{\rm orb}(\eta)$
($\rho = \ecRhoChifOrbChif$).  More tellingly, $\eta$ is
\emph{negatively} correlated with the excess
($\rho = \ecRhoExcessEta$, $p = \ecPExcessEta$), so the $L_{\rm orb}$
term dilutes rather than drives the Test~1 result.  Using the
progenitor spin component alone yields a stronger correlation
($\rho = \ecRhoExcessChifSpin$, $p = \ecPExcessChifSpin$); the partial
Spearman correlation controlling for~$\eta$ gives
$\rho = \ecRhoPartialChif$ ($p = \ecPPartialChif$); and the
correlation is highly significant within each of three equal-sized
$\eta$-tercile subsamples ($\rho \approx 0.45$--$0.55$ in each).
There is no $\eta$-confound: the excess tracks progenitor spin, not
mass-ratio geometry.

\paragraph{Test 2: deficit per unit $m_2$ vs.\ mass ratio}

Figure~\ref{fig:deficit_per_m2} plots the total mass deficit per
unit~$m_2$ against the mass ratio~$q$.  The singularity model curve
(red, dashed) is the spinning GR baseline (Eq.~\ref{eq:frac_gr})
evaluated at the sample median $\chi_{\rm eff} = \gwMedianChiEff$,
normalised by~$m_2$.  The quark star model curve (purple, solid) adds
the spin-induced $\Mg$ reduction,
$f_{\rm QS} = f_{\rm GR} + \alpha\,L_{\rm orb}(\eta)^{2}$,
using $\alpha = \gwAlphaQS$ taken directly from the Test~1 fit---a
parameter-free prediction for Test~2.  Both curves share the same
shape because GW radiation and spin deposition are both governed
by~$\eta$; the quark star curve is shifted uniformly upward because
the decreasing $L_{\rm orb}^{2}$ at low~$q$ is offset by the
increasing $1/m_2$ amplification.

The binned medians (green squares) track the quark star curve across
the full range of mass ratios, sitting consistently above the
singularity prediction ($\rho = \gwRhoDeficitQ$, $p = \gwPDeficitQ$).
The quark star model thus accounts for both tests with one physical
mechanism---centrifugal support reducing~$\Mg$---and one free
parameter ($\alpha$) calibrated on Test~1 and applied unchanged to
Test~2.
...
physical explanation.  We have argued that there is no baryonic mass
gap. The gap is in gravitational mass and is the signature of the
...
Massive pulsars are found exclusively at fast spin rates, consistent
with centrifugal support delaying collapse past the stability limit.
An assessment of the full observed pulsar population, for which mass
information is unavailable, shows that pulsars are disappearing from
the population, consistent with centrifugal support delaying collapse.
Failed supernovae confirm, independently of the pulsar evidence, that
stellar cores with baryonic mass well below $5\,\Msun$ collapse to
form black holes.  Finally, LIGO merger remnants show a spin-dependent
mass deficit that correlates with remnant spin and exceeds the
spinning GR prediction by the amount a structured-matter model
predicts---with a single free parameter calibrated on one observable
and applied to a second.

Several directions for future work are apparent.  Targeted mass
...
The author acknowledges the use of Claude (Anthropic) as an AI
assistant in the development, analysis, and manuscript preparation of
this work.
\end{acknowledgments}

\section*{Data Availability}

The data underlying this article are drawn from publicly available
catalogs: the ATNF Pulsar Catalogue \cite{Manchester2005}, the
GWTC-3 gravitational-wave catalog \cite{GWTC3,GWOSC2023}, and
equation-of-state tables from \texttt{stellarcollapse.org}
\cite{OConnorOtt2010}. Analysis code is available at
\url{https://github.com/glfrazier-42/MassGap}.

%% \bibliographystyle{apsrev4-2}
\bibliography{mass_gap}
